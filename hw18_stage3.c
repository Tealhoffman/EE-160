...
int main(void) {
  ...
  int i, j, k;                       // declare loop variables (i as row, j as column)
  ...
  // Calculating the resultant matrix F using i, j, and k "for" loops
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      F[i][j] = 0;                // Initialize F[i][j] to 0 to start the addition loop 
      for (k = 0; k < 3; k++) {
